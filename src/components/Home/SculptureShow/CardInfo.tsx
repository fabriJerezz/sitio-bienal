...
export default function CardInfo({pais,nombre,img,mov,historia,provincia,tec}: CardInfoProps) {
    ...
    <>
        <div onClick={handleClick} className='flex w-64 bg-primary z-3 flex-col relative rounded-lg border-secondary border p-2 transform transition-transform hover:scale-105 cursor-pointer'>
            <header className='relative  mb-2'>
                ...
                <h2 className='relative text-xl ml-3 italic'>{provincia}</h2>
            </header>
            <div className='relative w-full h-full'>
                <Image alt="Descripción de la imagen" src={esc} className='w-full h-full rounded-xl'></Image>
            </div>
       ...
       {showHidden && <HiddenInfo handleClick={handleClick} nombre={nombre} movimiento={mov} historia={historia} pais={pais} tec={tec}/>}
    </>
